...
#import besancon_model as besancon
...
keys = ['MAIN_ID','RA','DEC',\
        'FLUX_V','FLUX_G','FLUX_R','FLUX_J','FLUX_H','FLUX_K',\
        'FLUX_ERROR_V','FLUX_ERROR_G','FLUX_ERROR_R','FLUX_ERROR_J',\
        'FLUX_ERROR_H','FLUX_ERROR_K',\
        'SP_TYPE',\
        'PMRA','PMDEC','PM_ERR_MAJA','PM_ERR_MINA','PM_ERR_ANGLE','PM_QUAL','PM_BIBCODE',\
        'RV_VALUE','RVZ_BIBCODE','RVZ_RADVEL',\
        'PLX_VALUE','PLX_ERROR',\
        'RA_gal','DEC_gal']
...
path_root = Path(infolder_super_earths)
path_csv_tables = path_root.joinpath('csv_tables')
path_gaia = path_root.joinpath('Gaia')
pd_targets= pd.read_csv(path_csv_tables.joinpath('names.csv'),sep=';') # Celia: added sep=';', I think before the separator use to be ',' so i.e. the default one

fov_irdis_arcsec2 = 11*12.5
fov_ifs_arcsec2 = 1.73**2

target_of_interest= 'GJ674'

for i,target_name in enumerate(pd_targets['target name']):

    if target_name != target_of_interest: continue
    print(target_name)
    
    if target_name == 'BD061339':
        target_name = 'BD-06 1339'
    result_table_names = customSimbad.query_object(target_name)
    for key in keys:
        print(key,result_table_names[key][0])
        dico[key].append(result_table_names[key][0])
...
if False : #get besancon info (Celia commented)
...
pd_info_stars.to_csv(path_csv_tables.joinpath('target_properties.csv'),sep=';')
...
#     date_obs_str = pd_statistics['date start'][i_stat]
    ...
    print(len(pd_info_stars['target name']))
    print('0',pd_info_stars['target name'])
    print('1',pd_info_stars['target name']==target_name)
    
    print(pd_info_stars.columns)
    print('2',np.where(pd_info_stars['target name']==target_name))
    print('3',np.where(pd_info_stars['target name']==target_name)[0])
...
##pd_statistics.to_csv(path_csv_tables.joinpath('statistics_with_Gaia_contaminants.csv')) # Celia commented
