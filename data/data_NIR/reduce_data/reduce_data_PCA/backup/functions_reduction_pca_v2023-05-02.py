...
def run_and_collapse_pca(cube, pa, nmodes, psf, header=None, instru='IRDIS', idx_channel_beg=None, idx_channel_end=None,
                    technique = 'combine_before', mask_rad = 6, platescale=12.25, save_fits=1,
    ...
    if instru == 'IRDIS':
        ...
        normalization_factor = np.array([normalization(psf[i]) for i in range(idx_channel_beg, idx_channel_end)])

    print("The factors of normalization are:", normalization_factor)

    if technique == 'combine_after' :
        cube_im_pca, pcs = [], []
        nchannels = int(np.where(instru=='IRDIS',2,39))
        print(nchannels)
        ...
        cube_im_pca = np.concatenate(np.array(cube_im_pca))
        pcs =  np.concatenate(np.array(pcs))

        if save_fits :
            fits.writeto('{}fits/{}_cube_im_pca_4D.fits'.format(saving_dir, namesave), cube_im_pca, header, overwrite=True)
            fits.writeto('{}fits/{}_normalization_factors.fits'.format(saving_dir, namesave), normalization_factor, header, overwrite=True)

        cube_im_pca = np.nanmean( cube_im_pca, axis=0)
        pcs = np.nanmean( pcs, axis=0)
        ...
        if instru == 'IRDIS':
            ...
            print("Collapse data before running the PCA.")
            # Consider channels between channel beg and channel end
            print("Shape of the cube in 1 channel:", np.shape(cube[0]))
            for i in range(idx_channel_beg, idx_channel_end):
                cube[i] = cube[i]/normalization_factor[i-idx_channel_beg] 
            cube_collapse = np.nanmean(cube, axis=0)
            cube_im_pca, pcs = run_pca(cube_collapse, pa, nmodes, header=header, mask_rad = mask_rad,
                            save_fits=save_fits, plot_modes = plot_modes, plot_res_maps = plot_res_maps,
                            saving_dir = saving_dir, namesave = namesave,
...
def normalization(psf, crop_psf=25):
    print('In normalization function')
    print('Shape of the PSF before cropping:', np.shape(psf))
    psf = psf[:,crop_psf+1:-crop_psf,crop_psf+1:-crop_psf] # we crop to avoid negative values
    print('Shape of the PSF after cropping:', np.shape(psf))
    factor_normalization = np.nansum(psf)
    #print(factor_normalization)
    #fits.writeto('psf_cropped.fits',np.array(psf),overwrite=True)
