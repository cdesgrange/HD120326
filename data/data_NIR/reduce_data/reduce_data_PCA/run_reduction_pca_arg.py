...
fn_log = "{}/log_PCA_processing_{}".format('logs', fn[len('config_files/'):-6] )
...
IDX_CHANNEL_BEG, IDX_CHANNEL_END = [], []
...
for config in config_reductions: # each reduction has its own configuration
    ...
    IDX_CHANNEL_BEG.append(config['idx_channel_beg'])
    IDX_CHANNEL_END.append(config['idx_channel_end'])
...
for k in range(n):
    ...
    idx_channel_beg, idx_channel_end = IDX_CHANNEL_BEG[k], IDX_CHANNEL_END[k]
    ...
    save_fits, plot_modes, plot_res_maps, plot_data = 1, 0, 0, 0
    ...
    try :
        ...
        if channels == 'both' :
            ...
            cube_im_pca, pcs = run_and_collapse_pca(cube, pa, nmodes, psf, header, instru=instru, idx_channel_beg=idx_channel_beg, idx_channel_end=idx_channel_end,
                            technique=technique, mask_rad = mask_rad, platescale=platescale, save_fits=save_fits,
