...
#######################################################
def call_gen_disk_7params(theta):
...
def chisquare_7params(theta):
    ...
    Returns:
        Chisquare
    """
    model = call_gen_disk_7params(theta)
...
def logl_7params(theta):
    ...
    Returns:
        Chisquare
    """
    ll = -0.5 * chisquare_7params(theta)
    return ll



########################################################
def from_param_to_theta_init_7params(params_mcmc_yaml):
    ...
    ain = params_mcmc_yaml['ain_init']
    aout = params_mcmc_yaml['aout_init']
    
    #theta_init = (rad_init, pa_init, np.cos(np.radians(inc_init)), g1_init, scaling)
    theta_init = (rad_init, pa_init, inc_init, g1_init, ain, aout, scaling)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        str_yalm = 'SPHERE_Hband_AMOEBA_polar_7params.yaml'
    else:
        str_yalm = sys.argv[1]
    ...
    if 1:
        ...
        print('\n- No disk')
        startTime = datetime.now()
        theta_init = from_param_to_theta_init_7params(params_mcmc_yaml)
        ...
        if exploration_algo == "MCMC":
            ...
            chisquare_init_nodisk = chisquare_7params(theta_init_no_disk)
        ...
        if exploration_algo == "MCMC":
            ...
            chisquare_init = chisquare_7params(theta_init)
        ...
        disk_model = call_gen_disk_7params(theta_init)
    ...
    if exploration_algo == "MCMC":
        ...
        startTime = datetime.now()
        theta_init = from_param_to_theta_init_7params(params_mcmc_yaml)
        bounds = [(10,100),(0,180), (0, 89), (0,1), (0,None), (None,0), (0,None)] #params_mcmc_yaml['bounds']
        ...
        a = zip(*bounds)
        print(a)
        result_optim = op.minimize(chisquare_7params,
                                   theta_init,
                                   method='Nelder-Mead',
                                   #constraints=constr
                                   bounds=bounds
        ...
        best_model = call_gen_disk_7params(best_theta)
