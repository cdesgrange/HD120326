...
def logl_5params(theta):
    ...
    modelconvolved = modelconvolved/  np.nanmax(modelconvolved)
    ...
    #before:
    #return np.abs(Chisquare)

    #now: as we want to minimize chisquare?! ask Julien
    return np.abs(Chisquare)
...
def call_gen_disk(theta):
    ...
    if eccentricity == None: eccentricity_used = 0.001
    else: eccentricity_used = eccentricity

    if argperi == None: argperi_used = 0.001
    else: argperi_used = argperi

    if g2 == None: g_used = g1; name_spf = 'HG'
    else: g_used = [g1, g2]; name_spf = 'DoubleHG'
