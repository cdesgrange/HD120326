...
def chisquare_params_1obs(theta):
    ...
    if TYPE_OBS == 'total_intensity':
        # Generate disk model
        theta = np.array([theta[0],theta[1],theta[2],theta[3],theta[4],False])
...
def chisquare_params_2obs_polar(theta):
    """ measure the Chisquare of the parameter set.
        create disk
        convolve by the PSF (psf is global)
        do the forward modeling (diskFM obj is global)
    ...
    """
    Chisquare_red_final = []
    theta0 = np.array([theta[0],theta[1],theta[2],theta[3],theta[4],True])
    theta1 = np.array([theta[0],theta[1],theta[2],theta[3],theta[5],False])
    ...
    if SAVE_DETAIL_RESULTS:
        ...
        fits.writeto(os.path.join(detail_resultdir,'disk_model_convolved_2{}.fits'.format(suffix)),modelconvolved, overwrite=True)      
        fits.writeto(os.path.join(detail_resultdir,'residuals_2{}.fits'.format(suffix)), res_all, overwrite=True)    
...
########################################################
def chisquare_params_3obs_ti_only(theta):
    """ measure the Chisquare of the parameter set.
        create disk
        convolve by the PSF (psf is global)
        do the forward modeling (diskFM obj is global)
    ...
    """
    Chisquare_red_final = []
    theta0 = np.array([theta[0],theta[1],theta[2],theta[3],theta[4], False])
    theta1 = np.array([theta[0],theta[1],theta[2],theta[3],theta[5], False])
    theta2 = np.array([theta[0],theta[1],theta[2],theta[3],theta[6], False])
    ...
    i=2
    model = call_gen_disk_5params_ti(theta2)
