...
def chisquare_5params(theta):
    ...
    if TYPE_OBS == 'total_intensity':
        ...
        if SAVE_INTERMEDIATE_RESULTS:
            ...
            L = dt.seconds
            suffix = "_{:.2f}min_{:.0f}".format(L/60, Chisquare)

            modelconvolved0 = convolve_fft(model, PSF, boundary='wrap')

            fits.writeto(os.path.join(intermediate_resultdir,'residuals_fm{}.fits'.format(suffix)),
                 im_pca, overwrite=True)

            fits.writeto(os.path.join(intermediate_resultdir,'disk_model_convolved{}.fits'.format(suffix)),
        ...
        res = (SCIENCE_DATA_MASK - modelconvolved) / NOISE
        Chisquare = np.nansum(res * res)
...
def do_test_disk_first_guess():
    ...
    elif exploration_algo == "AMOEBA" and TYPE_OBS == 'polar':
    ...
    if TYPE_OBS == 'polar':
        ...
        modelconvolved0 = convolve_fft(model, PSF, boundary='wrap')
        best_residuals = (SCIENCE_DATA - modelconvolved0)
        ...
        # Generate disk model
        theta_init_no_pa = np.copy(THETA_INIT)
        #theta_init_no_pa[1] = 0

        if display: print('Check disk model parameters with no PA:\n', theta_init_no_pa)
        
        model = call_gen_disk_5params(THETA_INIT)
        modelconvolved0 = convolve_fft(model, PSF, boundary='wrap')
        
        model_nopa = call_gen_disk_5params(theta_init_no_pa)
        # Rotate the disk model for different angles
        # and convolve it by the PSF
        modelconvolved = vip.fm.cube_inject_fakedisk(model_nopa, -PA_ARRAY, psf=PSF)
       
        DIFF0 = RED_DATA - modelconvolved0
            
        # Remove the disk from the observation
        CUBE_DIFF = np.copy(SCIENCE_DATA) - np.copy(modelconvolved)
        ...
        fits.writeto(os.path.join(firstguess_resultdir,'first_guess_PCA_pcs_cube.fits'), pcs, overwrite=True)
        fits.writeto(os.path.join(firstguess_resultdir,'first_guess_PCA_reconstr_cube.fits'), reconstr_cube, overwrite=True)
        fits.writeto(os.path.join(firstguess_resultdir,'first_guess_PCA_res_cube.fits'), res_cube, overwrite=True)
        fits.writeto(os.path.join(firstguess_resultdir,'first_guess_PCA_res_cube_derot.fits'), res_cube_derot, overwrite=True)
        ...
        fits.writeto(os.path.join(firstguess_resultdir,'first_guess_disk_model_convolved_cube.fits'), modelconvolved, overwrite=True)
        fits.writeto(os.path.join(firstguess_resultdir,'first_guess_residuals_cube.fits'), CUBE_DIFF, overwrite=True)
        fits.writeto(os.path.join(firstguess_resultdir,'first_guess_residuals.fits'), DIFF0, overwrite=True)    

        fits.writeto(os.path.join(firstguess_resultdir,'first_guess_residuals_fm.fits'), best_residuals, overwrite=True)      
        fits.writeto(os.path.join(firstguess_resultdir,'first_guess_residuals_fm_snr.fits'), best_residuals_snr, overwrite=True)

    if display:  print('Save first-guess files')

    fits.writeto(os.path.join(firstguess_resultdir,'first_guess_disk_model_convolved.fits'),modelconvolved0, overwrite=True)      
    fits.writeto(os.path.join(firstguess_resultdir,'first_guess_disk_model{}.fits'.format(suffix)),model, overwrite=True)
...
if __name__ == '__main__':
    ...
    if exploration_algo == "MCMC":
        ...
        amoebaresultdir = SAVINGDIR +  '/amoeba_final'
    ...
    file_destination =  os.path.join(os.getcwd(),'results/log')
