...
def make_corner_plot(params_mcmc_yaml):
    ...
    chain_flat[:, 11] = np.exp(chain_flat[:, 11])
...
def create_header(params_mcmc_yaml):
    ...
    for modeli in range(chain_flat.shape[0]):
        r1_here = samples_dict['Rad'][modeli]

        samples_dict['Rmas'][modeli] = convert.au_to_mas(
...
def best_model_plot(params_mcmc_yaml, hdr):
    ...
    global PIXSCALE_INS, DISTANCE_STAR, DIMENSION
    ...
    theta_ml = (np.log(hdr['Rad_ML']), hdr['ain_ML'], hdr['aout_ML'],
                np.cos(np.radians(hdr['inc_ML'])), hdr['PA_ML'], hdr['Argpe_ML'],
                hdr['ecc_ML'], hdr['ksi0_ML'], hdr['g1_ML'], hdr['g2_ML'], hdr['Alph_ML'],
                np.log(hdr['Norm_ML']))
    ...
    psf = fits.getdata(os.path.join(DATADIR, file_prefix + '_PSF_convolution.fits'))
...
if __name__ == '__main__':
    ...
    if socket.gethostname() == 'macbookajm.home':
